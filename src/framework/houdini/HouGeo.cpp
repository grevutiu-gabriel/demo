...
namespace houdini
{
	...
	void HouGeo::addPrimitive( base::ScalarField::Ptr field )
	{
		...
		if( !pAttr )
		{
			...
			pAttr->m_attr = base::Attribute::createV4f();
	...
	HouGeo::HouAttribute::HouAttribute( const std::string &name, base::Attribute::Ptr attr ) : Attribute(),
	...
	*/
	...
	HouGeo::HouAttribute::Ptr HouGeo::loadAttribute( json::ArrayPtr attribute, sint64 elementCount )
	{
		...
		if( attrType == Attribute::ATTR_TYPE_NUMERIC )
		{
			...
			base::Attribute::ComponentType attrComponentType = base::Attribute::componentType(attrData->get<std::string>("storage"));
			int attrNumComponents = attrData->get<int>("size");
			attr->m_attr = std::make_shared<base::Attribute>( attrNumComponents, attrComponentType );
	...
	void HouGeo::loadVolumePrimitive( json::ObjectPtr volume )
	{
		...
		vol->field = std::make_shared<base::ScalarField>();
