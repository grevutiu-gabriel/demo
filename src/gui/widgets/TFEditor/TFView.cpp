...
TFView::TFView(VolumeWrapper::Ptr volume, QWidget* pParent /*= NULL*/) :
	...
	m_TransferFunctionItem(NULL)
{
	...
	m_TransferFunctionItem.SetTransferFunction(&volume->getTransferFunction());
	...
	m_Grid.setTransform(QTransform::fromTranslate(m_Margin.GetLeft(), m_Margin.GetTop()));
	m_HistogramItem.setTransform(QTransform::fromTranslate(m_Margin.GetLeft(), m_Margin.GetTop()));
	m_TransferFunctionItem.setTransform(QTransform::fromTranslate(m_Margin.GetLeft(), m_Margin.GetTop()));

	QObject::connect(&volume->getTransferFunction(), SIGNAL(Changed()), this, SLOT(OnTransferFunctionChanged()));
	QObject::connect(&volume->getTransferFunction(), SIGNAL(SelectionChanged(QNode*)), this, SLOT(OnNodeSelectionChanged(QNode*)));
}



void TFView::resizeEvent(QResizeEvent* pResizeEvent)
{
...
void TFView::SetHistogram(QHistogram& Histogram)
{
	m_HistogramItem.SetHistogram(Histogram);
}

void TFView::OnTransferFunctionChanged(void)
{
	m_TransferFunctionItem.Update();
}

void TFView::OnNodeSelectionChanged(QNode* pNode)
{
	// Deselect all nodes
...
void TFView::setEnabled(bool Enabled)
{
...
//void TFView::mousePressEvent(QMouseEvent* pEvent)
//{
