...
namespace base
{
	...
	std::map<std::string, Path> g_pathRegistry;
	// stores a specific path in registry using the given key
	void pathRegister( const std::string &key, Path p )
	{
		g_pathRegistry[key] = p;
	}

	// returns true if a path has been registered already under the given key
	bool pathIsRegistered( const std::string &key )
	{
		std::map<std::string, Path>::iterator it = g_pathRegistry.find( key );
		if( it != g_pathRegistry.end() )
			return true;
		return false;
	}
