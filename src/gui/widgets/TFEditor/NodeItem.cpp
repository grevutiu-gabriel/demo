...
void QNodeItem::mousePressEvent(QGraphicsSceneMouseEvent* pEvent)
{
	...
	{
		if (m_pTransferFunctionItem->getTransferFunction()->GetNodes().indexOf(*m_pNode) == 0 || m_pTransferFunctionItem->getTransferFunction()->GetNodes().indexOf(*m_pNode) == m_pTransferFunctionItem->getTransferFunction()->GetNodes().size() - 1)
		{
