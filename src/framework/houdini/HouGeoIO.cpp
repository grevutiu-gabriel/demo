...
namespace houdini
{
	...
	base::Geometry::Ptr HouGeoIO::convertToGeometry(HouGeo::Ptr houGeo, int prim )
	{
		...
		if( !houPrim )
		{
			...
			return base::GeometryPtr();
		}

		// cast and get some first numbers...
		HouGeo::HouPoly::Ptr poly = base::dynamic_pointer_cast<HouGeo::HouPoly>(houPrim);
		...
		if( prim == -1 )
			result = base::Geometry::createPointGeometry();
		else
		if( numVerticesPerPoly == 3 )
			result = base::Geometry::createTriangleGeometry();
		else
		if( numVerticesPerPoly == 4 )
			result = base::Geometry::createQuadGeometry();

		// attributes ---
		std::vector<base::AttributePtr> geoAttrs;
		...
		for( std::vector<std::string>::iterator it = pointAttributesNames.begin(), end = pointAttributesNames.end();  it != end; ++it )
		{
			...
			HouGeoAdapter::Attribute::Ptr houAttr = houGeo->getPointAttribute(attrName);

			int numComponents = houAttr->getTupleSize();

			base::AttributePtr attr;
			if( attrName == "P" )
			{
			...
			if( houAttr->getStorage() == HouGeoAdapter::Attribute::ATTR_STORAGE_FPREAL32 )
				attr = base::Attribute::create( numComponents, base::Attribute::FLOAT, (unsigned char *)houAttr->getRawPointer()->ptr, houAttr->getNumElements() );

			result->setAttr( attrName, attr );
		}

		// convert vertex attributes ---
		std::vector<std::pair<base::Attribute::Ptr, base::Attribute::Ptr>> vertex2pointAttr;
		for( std::vector<std::string>::iterator it = vertexAttributesNames.begin(), end = vertexAttributesNames.end();  it != end; ++it )
		{
			std::string attrName = *it;
			HouGeoAdapter::Attribute::Ptr houAttr = houGeo->getVertexAttribute(attrName);
			...
			base::AttributePtr pointAttr = attr->copy();
		...
		if( prim > -1 )
		{
			...
			for( int i=0;i<numVertices;++i )
			{
				...
				if( !pointsToSplit[point] )
					...
					{
						// compare
						for( std::vector<std::pair<base::AttributePtr, base::AttributePtr>>::iterator it = vertex2pointAttr.begin(), end = vertex2pointAttr.end(); it != end; ++it )
