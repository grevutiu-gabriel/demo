...
namespace base
{
	...
	struct OrbitNavigator
	{
		...
		OrbitNavigator( Camera::Ptr camera );  // constructor

		void                          update();  ///< recomputes the transform from current polar coordinates and distance value
		void                          fitView( const math::BoundingBox3f &bound ); // will adjust lookat and distance such that given bound is within view
